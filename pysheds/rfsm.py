...
class RFSM:
    ...
    def find_watersheds(self):
        ...
        for index in range(len(self.levels) - 1):
            ...
            w = skimage.segmentation.watershed(self.dem, self.levels[index], 
    ...
    def find_connections(self):
        ...
        for index in range(len(self.levels) - 1):
            ...
            width = self.ws[index].shape[1]
            offsets = np.array([-width - 1, -width, -width + 1, -1, 1, width - 1, width, width + 1])
            neighbors_indices = np.clip(comm[:, None] + offsets, 0, self.ws[index].size - 1)
            neighbors = self.ws[index].flat[neighbors_indices]
    ...
    def find_drop(self):
        ...
        for index, n in enumerate(self.ns):
            subnum.append(np.arange(1, n + 1))
            full.append(np.arange(1, n + 1) + n_i)
            level.append(np.repeat(index, n))
            n_i += n
        self.dropmap = pd.DataFrame(np.column_stack([np.concatenate(level), np.concatenate(subnum)]), 
                                    index=np.concatenate(full))
        drop = skimage.segmentation.watershed(self.dem, np.where(self.ws[0] > 0, self.ws[0], 0), mask=self.ws[1] > 0)
        for index in range(1, len(self.ws) - 1):
            base = drop + np.where((self.ws[index] > 0) & (drop == 0), sum(self.ns[:index]) + self.ws[index], 0)
            drop = skimage.segmentation.watershed(self.dem, base, mask=self.ws[index + 1] > 0)
        self.drop = drop

    def create_tree(self):
        self.nodes = []
        for index in range(len(self.levels)):
            self.nodes.append({i: Node(name=i, level=index) for i in range(1, self.ns[index] + 1)})
        self.tmap = []
        for index in range(len(self.levels) - 1):
            g = set(range(1, self.ns[index] + 1))
            s = pd.DataFrame(pd.Series(self.c[index]))
            s[0], s[1] = s[0].astype(int), self.dem.flat[s[0].values.astype(int)]
            s[2] = [self.lup[index][i] for i in s.index.get_level_values(0)]
            s = s.sort_values([2,1])
            num_connections = dict(s.groupby(2).size())
            self.tmap.append({})
            for i in s.index:
                l, r = self.nodes[index][i[0]], self.nodes[index][i[1]]
                if not (self.get_root(l) is self.get_root(r)):
                    parent = Node(comm=s.loc[i, 0], elev=s.loc[i, 1], level=index)
                    ...
                    if num_connections[upper_label] == 0:
                        parent.level, parent.name = (index + 1), upper_label
                        self.nodes[index + 1][parent.name] = parent
                else:
                    num_connections[int(s.loc[i, 2])] -= 1
            if g:
                for j in g:
                    upper_label = self.lup[index][j]
                    child, parent = self.nodes[index][j], self.nodes[index + 1][upper_label]
                    parent.elev = self.dem[self.levels[index + 1] == upper_label].min().item()
                    child.parent, parent.l = parent, child
    ...
    def check_overflow(self, node, overflowing, break_level=-1):
        if node.level > break_level:
            if node.current_vol > (node.vol + 1e-9):
                overflowing[0] = True
            if not overflowing[0]:
                if node.l: self.check_overflow(node.l, overflowing, break_level=break_level)
                if node.r: self.check_overflow(node.r, overflowing, break_level=break_level)

    def spill(self):
        for i in range(self.max_spills):
            overflowing = [False]
            self.check_overflow(self.root, overflowing)
            if not overflowing[0]:
                print(f"Algorithm stabilized in {i} iterations.")
                break
        ...
        self.check_overflow(self.root, overflowing)
        if overflowing[0]:
            print(f"Warning: Overflow remains after {self.max_spills} iterations.")

    def spread_volumes(self, node):
        if (node.current_vol > (node.vol + 1e-9)) and (node.t):
            transfer = node.current_vol - node.vol
            node.current_vol -= transfer
            node.t.current_vol += transfer
        if node.l: self.spread_volumes(node.l)
        if node.r: self.spread_volumes(node.r)
        if node.l:
            l_full, r_full = self.node_full(node.l), (self.node_full(node.r) if node.r else True)
            if l_full and r_full:
                ltransfer = node.l.current_vol - node.l.vol
                rtransfer = (node.r.current_vol - node.r.vol) if node.r else 0
    ...
    def set_node_transfers(self):
        for index, mapping in enumerate(self.tmap):
            for pair, node in mapping.items():
                ...
                offsets = np.array([-width - 1, -width, -width + 1, -1, 1, width - 1, width, width + 1])
                neighbors = np.clip(int(node.comm) + offsets, 0, self.dem.size - 1).astype(int)
                ser = pd.DataFrame(np.column_stack([neighbors, self.dem.flat[neighbors], self.ws[index].flat[neighbors]]))
                ser = ser[ser[2].isin(list(pair))]
                g_idx = ser.groupby(2).idxmin()[1].apply(lambda x: ser.loc[x, 0])
                fullix = self.drop.flat[g_idx.values.astype(int)]
                g_dict = pd.DataFrame(np.column_stack([self.dropmap.loc[fullix][0].values, self.dropmap.loc[fullix][1].values]), 
                                     index=g_idx.index.values.astype(int)).to_dict(orient='index')
                node.l.t = self.nodes[g_dict[j][0]][g_dict[j][1]]
                node.r.t = self.nodes[g_dict[i][0]][g_dict[i][1]]
    ...
    def compute_vol(self, z, node, target_vol):
        ...
        if node.name:
            ...
            boundary = list(chain.from_iterable([self.b[node.level].setdefault(pair, []) for pair in combinations(leaves, 2)]))
    ...
    def get_root(self, node):
        return self.get_root(node.parent) if node.parent else node

    def enumerate_leaves(self, node, level=0, stack=[]):
        if node.level >= level:
            if node.name: stack.append(node.name)
            if node.l: self.enumerate_leaves(node.l, level=level, stack=stack)
            if node.r: self.enumerate_leaves(node.r, level=level, stack=stack)

    def set_cumulative_capacities(self, node):
        if node.l: self.set_cumulative_capacities(node.l)
        if node.r: self.set_cumulative_capacities(node.r)
        if node.parent:
            if node.name:
                mask = (self.ws[node.level] == node.name)
            else:
            ...
            node.vol = abs(elevdiff[elevdiff > 0].sum().item() * self.x * self.y)

    def set_marginal_capacities(self, node):
        if node.l:
            node.cumulative_vol = node.vol
            node.vol = max(node.vol - node.l.vol - (node.r.vol if node.r else 0), 0)
            self.set_marginal_capacities(node.l)
            if node.r: self.set_marginal_capacities(node.r)
        else:
            node.cumulative_vol = node.vol

    def set_singleton_transfer(self, node):
        if node.l: self.set_singleton_transfer(node.l)
        if node.r: self.set_singleton_transfer(node.r)
        if node.parent and node.parent.l and not node.parent.r:
            node.t = node.parent

    def node_full(self, node):
        if node.vol == 0:
            full_arr = [True]
            self.check_full(node, full_arr)
            return full_arr[0]
        return node.current_vol >= node.vol

    def check_full(self, node, full_arr):
        if node.vol != 0:
            full_arr[0] &= (node.current_vol >= node.vol)
        if full_arr[0]:
            if node.l: self.check_full(node.l, full_arr)
            if node.r: self.check_full(node.r, full_arr)

    def volume_to_level(self, node, waterlevel):
        if node.current_vol > 0:
            maxelev = node.parent.elev
            minelev = node.elev if node.elev else np.nanmin(self.dem)
            target_vol = node.current_vol
    
            # 1. Check signs before bisecting to avoid ValueError
            # f(a) is always negative (-target_vol)
            fa = -target_vol
            # f(b) is capacity at maxelev - target_vol
            fb = self.compute_vol(maxelev, node, target_vol)
            
            if target_vol <= 0:
                elev = minelev
            elif fb <= 0:
                # If target volume is greater than or equal to capacity, 
                # set level to the lip (maxelev)
                elev = maxelev
            else:
                try:
            ...
            if node.name:
                ...
                boundary = list(chain.from_iterable([self.b[node.level].setdefault(pair, []) for pair in combinations(leaves, 2)]))
                mask.flat[boundary] = True
            waterlevel.flat[np.flatnonzero(mask & (self.dem < elev))] = elev
        else:
            if node.l: self.volume_to_level(node.l, waterlevel)
            if node.r: self.volume_to_level(node.r, waterlevel)
    ...
    def remove_volume(self, node):
        ...
        if node.l: self.remove_volume(node.l)
        if node.r: self.remove_volume(node.r)